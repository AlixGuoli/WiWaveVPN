...
/// EM Int 加载通道：与原版 Y Int 逻辑一致，仅 key 来源改为 Yandex_EMInt_List。
final class FluxEMIntChannel: NSObject {
    private let channelTag = "AdEM"
    ...
    func expose(from host: UIViewController, trigger: FluxAdTrigger? = nil) {
        ...
        ad.delegate = self
        ad.show(from: host)
        AppLogger.log(.system, tag: channelTag, "开始展示 EM Int trigger=\(trigger?.rawValue ?? "nil")")
    }
    ...
    private func reloadSlotKeys() {
        slotIDs = QuillAdConfigCache.shared.slotIDs(for: .emInterstitial)
        if slotIDs.isEmpty {
            AppLogger.log(.system, tag: channelTag, "未配置 Yandex_EMInt_List")
        }
    }
...
extension FluxEMIntChannel: InterstitialAdLoaderDelegate {
...
extension FluxEMIntChannel: InterstitialAdDelegate {
